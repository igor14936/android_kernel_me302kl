...
static int hdmi_msm_read_edid(void)
{
	...
	if (external_common_state->pre_suspend_hpd_state) {
		printk("%s: pre_suspend_hpd_state=%d, sleep 500 ms.\n",
			__func__, external_common_state->pre_suspend_hpd_state);
		msleep(500);
		external_common_state->pre_suspend_hpd_state = false;
	}
...
error:
	if (hdmi_msm_state->hpd_during_auth) {
		...
		/* If reauth is required, set hdcp_activating to FALSE before
		queuing the hdcp_reauth_work. */
		mutex_lock(&hdmi_msm_state_mutex);
		hdmi_msm_state->hdcp_activating = FALSE;
		mutex_unlock(&hdmi_msm_state_mutex);
...
static int hdmi_msm_hpd_on(void)
{
	...
	if (hdmi_msm_state->hpd_initialized) {
		...
		/* Add HDMI hotplug wakeup source */
		enable_irq_wake(gpio_to_irq(HDMI_HPD_GPIO));
	}
