...
#ifdef CONFIG_CHARGER_SMB345
static void report_cable_type(enum msm_otg_smb345_chg_type chg_src)
{
		usb_cable_type_detect(chg_src);
}
#endif
...
static void ulpi_init(struct msm_otg *motg)
{
	...
	while (seq[0] >= 0) {
		...
		if (otg_host_on == 1 && seq[1] == 0x81 && machine_is_apq8064_duma()) {
			printk(KERN_INFO"host mode  0x81 dc level seq[1] = %x\n",seq[1]);
