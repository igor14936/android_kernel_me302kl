...
static int mi1040_regulator_init(bool on)
{
	...
	if (on) {
		...
		if (!reg_8921_l8) {
			reg_8921_l8 = regulator_get(&mi1040_s_ctrl.
...
int32_t mi1040_sensor_set_ev(struct msm_sensor_ctrl_t *s_ctrl, int compensation)
{
	...
	if (ev_value != compensation) {
		...
		case CAMERA_EXPOSURE_COMPENSATION_LV4:
			CDBG("--CAMERA--CAMERA_EXPOSURE_COMPENSATION_LV4\n");
			rc = sensor_write_table(s_ctrl, EV_minus_2, ARRAY_SIZE(EV_minus_2));
			break;
...
static int mi1040_gpio_request(void)
{
	...
	if(machine_is_apq8064_flo() || machine_is_apq8064_deb()){
		...
		rc = gpio_request(CAM_RST_DUMA, "mi1040");
		if (rc) {
			pr_err("%s: gpio CAM_RST_DUMA %d, rc(%d)fail\n", __func__, CAM_RST_DUMA, rc);
			goto init_probe_fail;
		}
		rc = gpio_request(CAM_PWDN, "mi1040");
		if (rc) {
...
int32_t mi1040_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(machine_is_apq8064_flo() || machine_is_apq8064_deb()){
		...
		rc = pm8xxx_gpio_config(CAM_RST_DUMA, &pm_isp_gpio_low);
		if (rc != 0)
			pr_err("PMIC gpio 12 CAM_RST config low fail\n");
		else
			pr_info("PMIC gpio 12 CAM_RST(%d)\n", gpio_get_value(CAM_RST_DUMA));
	...
	if(machine_is_apq8064_flo() || machine_is_apq8064_deb()){
		...
		rc = pm8xxx_gpio_config(CAM_RST_DUMA, &pm_isp_gpio_high);
		if (rc != 0)
			pr_err("PMIC gpio 12 CAM_RST config high fail\n");
		else
			pr_info("PMIC gpio 12 CAM_RST(%d)\n", gpio_get_value(CAM_RST_DUMA));
		msleep(5);
	...
	*ppos = len;
	...
	case 2:
			...
			snprintf(mi1040_s_ctrl.sensordata->vendor_name,
				sizeof(mi1040_s_ctrl.sensor_v4l2_subdev.name),
				"%s", "A");
			mi1040_s_ctrl.msm_sensor_reg->init_settings =
				mi1040_init_conf;
			break;
	case 3:
