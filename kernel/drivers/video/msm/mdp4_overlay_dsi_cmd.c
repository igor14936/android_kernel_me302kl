...
static void mdp4_dsi_cmd_do_blt(struct msm_fb_data_type *mfd, int enable)
{
	...
	if (need_wait) {
		pr_debug("%s: enable = %d ,  ov_blt_addr = 0x%x \n", __func__ ,enable ,(uint16)pipe->ov_blt_addr );
...
void mdp4_dsi_cmd_wait4vsync(int cndx)
{
	...
	if (ret <= 0) {
		...
		pr_err("%s MDP int =%x", __func__, inpdw(MDP_BASE + 0x50));
		pr_err("%s MDP int status =%x", __func__, inpdw(MDP_BASE + 0x54));
	}
...
static void mdp4_dsi_cmd_wait4dmap(int cndx)
{
	...
	pr_debug("%s: MDP_INTR_ENABLE = 0x%x    MDP_INTR_STATUS  = 0x%x   frame count = 0x%x   \r\n",
				__func__ , (int)inpdw(MDP_BASE + 0x50),
				(int)inpdw(MDP_BASE + 0x54), (int)inpdw(MDP_BASE + 0x140));
