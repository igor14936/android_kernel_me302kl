...
static char MCAP[2] = {0xB0, 0x00}; /* DTYPE_DCS_WRITE2 */
static char interface_setting[6] = {0xB3, 0x04, 0x08, 0x00, 0x22, 0x00}; /* DTYPE_GEN_LWRITE */
static char interface_ID_setting[2] = {0xB4, 0x0C}; /* DTYPE_GEN_LWRITE */
static char DSI_control[3] = {0xB6, 0x3A, 0xD3}; /* DTYPE_GEN_LWRITE */
static char set_pixel_format[2] = {0x3A, 0x77}; /* DTYPE_DCS_WRITE1 */
static char set_column_addr[5] = {0x2A, 0x00, 0x00, 0x04, 0xAF}; /* DTYPE_DCS_LWRITE */
static char set_page_addr[5] = {0x2B, 0x00, 0x00, 0x07, 0x7F}; /* DTYPE_DCS_LWRITE */
//static char write_memory[13] = {0x2C, 0xFF, 0x00, 0x00, 0xFF, 0x00, 0x00, 0xFF, 0x00, 0x00, 0xFF, 0x00, 0x00}; /* DTYPE_DCS_LWRITE */
static char write_display_brightness[] = {0x51, 0x00}; //DTYPE_DCS_WRITE1	//set brightness
static char write_control_display[] = {0x53, 0x24}; //DTYPE_DCS_WRITE1	//enable LEDPWM pin output, turn on LEDPWM output, turn off pwm dimming
static char write_cabc[] = {0x55, 0x03}; //DTYPE_DCS_WRITE1	//choose cabc mode, 0x00(-0%), 0x01(-15%), 0x02(-40%), 0x03(-54%), disable SRE(sunlight readability enhancement)
static char backlight_control1[] = {0xB8, 0x07, 0x87, 0x26, 0x18, 0x00, 0x32}; //DTYPE_GEN_LWRITE	//for cabc mode 0x1(-15%)
static char backlight_control2[] = {0xB9, 0x07, 0x75, 0x61, 0x20, 0x16, 0x87}; //DTYPE_GEN_LWRITE	//for cabc mode 0x2(-40%)
static char backlight_control3[] = {0xBA, 0x07, 0x70, 0x81, 0x20, 0x45, 0xB4}; //DTYPE_GEN_LWRITE	//for cabc mode 0x3(-54%)
static char backlight_control4[] = {0xCE, 0x7D, 0x40, 0x48, 0x56, 0x67, 0x78, 0x88, 0x98,
				0xA7, 0xB5, 0xC3, 0xD1, 0xDE, 0xE9, 0xF2, 0xFA,
				0xFF, 0x04, 0x00, 0x0F, 0x0F, 0x42, 0x00}; //DTYPE_GEN_LWRITE	//for pwm frequency and dimming control

static struct dsi_cmd_desc JDI_display_on_cmds[] = {
	{DTYPE_DCS_WRITE, 1, 0, 0, 5, sizeof(sw_reset), sw_reset},
	{DTYPE_GEN_WRITE2, 1, 0, 0, 0, sizeof(MCAP), MCAP},
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(interface_setting), interface_setting},
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(interface_ID_setting), interface_ID_setting},
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(DSI_control), DSI_control},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, sizeof(set_pixel_format), set_pixel_format},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(set_column_addr), set_column_addr},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(set_page_addr), set_page_addr},
//	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(write_memory), write_memory},
...
static struct dsi_cmd_desc JDI_display_on_cmds_command_bl[] = {
	...
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(backlight_control1), backlight_control1},
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(backlight_control2), backlight_control2},
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(backlight_control3), backlight_control3},
	{DTYPE_GEN_LWRITE, 1, 0, 0, 0, sizeof(backlight_control4), backlight_control4},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, sizeof(set_pixel_format), set_pixel_format},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(set_column_addr), set_column_addr},
	{DTYPE_DCS_LWRITE, 1, 0, 0, 0, sizeof(set_page_addr), set_page_addr},
	{DTYPE_DCS_WRITE, 1, 0, 0, 120, sizeof(exit_sleep), exit_sleep},
...
static char bl_value[2] = {0x51, 0x0};	/* DTYPE_DCS_WRITE1 */
...
static int mipi_JDI_lcd_on(struct platform_device *pdev)
{
	...
	if (hw_revision != 0x3) {
		...
		cmdreq_JDI.cmds_cnt = ARRAY_SIZE(JDI_display_on_cmds_command_bl);
	}
	...
	printk("%s, JDI display on command-\n", __func__);
...
static int mipi_JDI_lcd_off(struct platform_device *pdev)
{
	...
	printk("%s, JDI display off command+\n", __func__);
