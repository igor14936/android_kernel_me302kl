...
static int reset_store(const char *val, struct kernel_param *kp)
{
	int enable, value;

	pr_info("%s++\n", __func__);

	if (sscanf(val, "%u", &enable) != 1) {
		pr_err("value should be an unsigned integer\n");
		return -EINVAL;
	}

	if (enable == 0) {
		pr_err("value should not be zero\n");
		return -EINVAL;
	}
	...
	if (value == 0 || mdm_drv->mdm_ready == 0) {
		pr_err("%s: mdm_status = %d, mdm_ready = %d, return\n",
			__func__, value, mdm_drv->mdm_ready);
...
module_param_call(mdm_reset, reset_store, NULL, NULL, 0644);
