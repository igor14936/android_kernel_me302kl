...
#ifndef DISABLE_TTY_AT
	if (is_tty_at_opened()) {
		tty_data_bridge_process_rx(work, get_timestamp());
		return;
	}
#endif
...
{
	return submit_rx_urb(dev, rx_urb, flags);
}
...
int data_bridge_write(unsigned int id, struct sk_buff *skb)
{
	...
	if (!dev || !(dev->brdg || is_tty_at_opened()) || dev->err || !usb_get_intfdata(dev->intf))
		return -ENODEV;
