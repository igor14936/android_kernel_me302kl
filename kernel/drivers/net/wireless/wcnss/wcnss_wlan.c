...
extern void wcnss_s2_off(void);
extern int wcnss_s2_on(struct device *dev);
...
void wcnss_s2_power(int on)
{
	if (on)
		wcnss_s2_on(wcnss_wlan_get_device());
	else
		wcnss_s2_off();
}
EXPORT_SYMBOL(wcnss_s2_power);
