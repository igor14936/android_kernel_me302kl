...
#define gpio_EN_VDD_SYS 32
#define gpio_EN_VDD_BL PM8921_GPIO_PM_TO_SYS(23)		//EN_VDD_BL
#define gpio_LCD_BL_EN PM8921_GPIO_PM_TO_SYS(30)		//LCD_BL_EN
#define gpio_LCD_BL_PWM PM8921_GPIO_PM_TO_SYS(26)	//PWM
#define gpio_EN_VDD_PNL PM8921_GPIO_PM_TO_SYS(43)	//EN_VDD_PNL
...
static uint32_t duma_display_gpio_table[] = {
	/* EN_VDD_SYS */
	GPIO_CFG(gpio_EN_VDD_SYS, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};
...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		//EN_VDD_SYS, only for duma
		rc = gpio_request(gpio_EN_VDD_SYS, "EN_VDD_SYS");
		if (rc) {
			pr_err("request gpio 32 failed, rc=%d\n", rc);
			return -ENODEV;
		}
		//config EN_VDD_SYS
		for (n = 0; n < ARRAY_SIZE(duma_display_gpio_table); n++) {
			rc = gpio_tlmm_config(duma_display_gpio_table[n], GPIO_CFG_ENABLE);
			if (rc) {
				pr_err("%s: gpio_tlmm_config(%#x)=%d\n",
					__func__, duma_display_gpio_table[n], rc);
				break;
			}
		}

		// EN_VDD_PNL (3.3V)
		rc = gpio_request(gpio_EN_VDD_PNL, "EN_VDD_PNL");
		if (rc) {
			pr_err("request PMIC gpio 43 failed, rc=%d\n", rc);
			return -ENODEV;
		}
	...
	if (on) {
		...
		gpio_set_value_cansleep(gpio_EN_VDD_SYS, 1);
		gpio_set_value_cansleep(gpio_EN_VDD_PNL, 1);

		if(type==0) {
			printk("lcd_id=%d (CPT panel), delay 100 ms\n", type);
			msleep(100);
		} else {
			printk("lcd_id=%d (AUO panel), delay 20 ms\n", type);
			mdelay(20);
		}
	} else {

		mdelay(20);
		gpio_set_value_cansleep(gpio_EN_VDD_SYS, 0);
		gpio_set_value_cansleep(gpio_EN_VDD_PNL, 0);
...
static int mipi_auo_gpio[] = {LPM_CHANNEL};

static struct mipi_dsi_panel_platform_data mipi_auo_pdata = {
	.gpio = mipi_auo_gpio,
	.recovery_backlight = 102,	// 255 * 40%, follows bootloader
};

static struct platform_device mipi_auo_panel_device = {
	.name = "mipi_orise",
	.id = 0,
	.dev = {
		.platform_data = &mipi_auo_pdata,
	}
};
...
static int hdmi_enable_5v(int on)
{
// Comment this for duma since HDMI 5V power is controlled by HW
// SW does not need to control it.
#if 0
...
void __init apq8064_init_fb(void)
{
	...
	printk("Duma device, register mipi_auo_panel_device.\n");
	platform_device_register(&mipi_auo_panel_device);
