...
{
	/*
	 * FIXME: Workaround on Sandisk 64G sdcard
	 * always keep SD CLK running
	 */
	if (host->mmc && !strcmp(mmc_hostname(host->mmc), "mmc1")
		&& host->mmc->card
		&& (host->mmc->card->cid.manfid == 0x3)
		&& (host->mmc->card->cid.hwrev == 0x8)
		&& (host->mmc->card->cid.oemid == 0x5344)
		&& (!strcmp(host->mmc->card->cid.prod_name, "SU64G")))
		return 0;
...
#define MSM_MMC_DEFAULT_CPUDMA_LATENCY 200 /* usecs */
	...
	if (plat->status_irq) {
		...
		if (ret)
			pr_err("%s: SD card wake-up event registration failed with error: %d\n", mmc_hostname(mmc), ret);
