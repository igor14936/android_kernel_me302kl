...
static struct vregs_info iris_vregs_riva1[] = {
	{"iris_vddrfa1", VREG_NULL_CONFIG, 1300000, 0, 1300000, 100000, NULL},
};
...
void wcnss_s2_off(void)
{
	wcnss_vregs_off(iris_vregs_riva1, ARRAY_SIZE(iris_vregs_riva1));
}
EXPORT_SYMBOL(wcnss_s2_off);

int wcnss_s2_on(struct device *dev)
{
	return wcnss_vregs_on(dev, iris_vregs_riva1,ARRAY_SIZE(iris_vregs_riva1));
}
EXPORT_SYMBOL(wcnss_s2_on);
