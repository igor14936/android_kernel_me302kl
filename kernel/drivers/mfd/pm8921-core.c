...
struct pm8921 *temp_pm8921 = NULL;
...
static ssize_t show_pm8921_ssbi_status(struct device *dev, struct device_attribute *devattr, char *buf)
{
       if(temp_pm8921)
               return sprintf(buf, "%d\n", temp_pm8921->ssbi_status);
       else
               return sprintf(buf, "%d\n", 0);
}
static DEVICE_ATTR(pm8921_ssbi_status, S_IWUSR | S_IRUGO, show_pm8921_ssbi_status,NULL);

static struct attribute *pm8921_ssbi_attributes[] = {

       &dev_attr_pm8921_ssbi_status.attr,
       NULL,
};

static const struct attribute_group pm8921_ssbi_group = {
       .attrs = pm8921_ssbi_attributes,
};
...
static int __devinit pm8921_probe(struct platform_device *pdev)
{
	...
	temp_pm8921 = pmic;
	temp_pm8921->ssbi_status = 1;
	if (sysfs_create_group(&pdev->dev.kobj, &pm8921_ssbi_group)) {
		pr_err("Cannot to create the sysfs\n");
	}
