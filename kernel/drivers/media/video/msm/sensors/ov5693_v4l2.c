	...
	struct msm_calib_af *af_pos)
{
	...
	/* select otp bank 3 */
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client, OTP_BANK_SELECT, 0xc3,
		MSM_CAMERA_I2C_BYTE_DATA);
	/* load otp */
	...
	if (af_pos->inf_dac == 0) {
		/* select otp bank 2 */
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			OTP_BANK_SELECT, 0xc2, MSM_CAMERA_I2C_BYTE_DATA);
		/* load otp */
		msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			OTP_LOAD_DUMP, 0x01, MSM_CAMERA_I2C_BYTE_DATA);
		/* read inf */
		address = OTP_BANK_START;
...
static int ov5693_regulator_init(bool on)
{
	...
	if (on) {
		...
		if (!reg_8921_l8) {
			reg_8921_l8 = regulator_get(&ov5693_s_ctrl.
		...
		if (!reg_8921_l16) {
			reg_8921_l16 = regulator_get(&ov5693_s_ctrl.
...
static int ov5693_gpio_request(void)
{
	...
	if(machine_is_apq8064_flo() || machine_is_apq8064_deb()){
		...
		rc = gpio_request(PM8921_GPIO_PM_TO_SYS(11), "ov5693");
		if (rc) {
...
int32_t ov5693_sensor_read_vendor_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	msm_camera_i2c_read_seq(s_ctrl->sensor_i2c_client, OTP_BANK_START,
		ov5693_afcalib_data, 10);
		...
		const struct i2c_device_id *id)
{
	...
	sensor_read_reg(ov5693_s_ctrl.sensor_i2c_client->client,
		0x300A, &rdata);
...
static ssize_t ov5693_proc_read_camera_status(char *page,
...
case 2:
		{
			...
			snprintf(ov5693_s_ctrl.sensordata->vendor_name,
				sizeof(ov5693_s_ctrl.sensor_v4l2_subdev.name),
				"%s", "A");
			break;
		}
case 3:
	...
	size_t count, loff_t *ppos)
{
	...
	err = i2c_transfer(ov5693_s_ctrl.sensor_i2c_client->
...
static int i2c_read_otp_value(struct file *file, char __user *buf,
	size_t count, loff_t *ppos)
{
	int len = 0;
	char *bp = debugTxtBuf;
	...
	len = snprintf(bp, DBG_TXT_BUF_SIZE,
		"0x%X 0x%X 0x%X 0x%X 0x%X\n",
		ov5693_afcalib_data[0], ov5693_afcalib_data[1],
		ov5693_afcalib_data[2], ov5693_afcalib_data[3],
		ov5693_afcalib_data[4]);

	len += snprintf(bp + len, DBG_TXT_BUF_SIZE - len,
		"0x%X 0x%X 0x%X 0x%X 0x%X\n",
		ov5693_afcalib_data[5], ov5693_afcalib_data[6],
		ov5693_afcalib_data[7], ov5693_afcalib_data[8],
		ov5693_afcalib_data[9]);
...
static const struct file_operations read_otp_value = {
	.open = i2c_set_open,
	.read = i2c_read_otp_value,
};
...
static int __init qualcomm_i2c_debuginit(void)
{
	...
	(void) debugfs_create_file("read_otp_value", S_IRUGO | S_IWUSR,
			dent, NULL, &read_otp_value);
