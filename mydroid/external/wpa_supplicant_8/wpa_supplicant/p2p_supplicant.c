...
static int is_channel_avaliable(struct wpa_supplicant *wpa_s, struct wpa_scan_results *scan_res, int channel)
{
	size_t i;
	int freq;
		...
		if (mode->channels[i].chan == chan) {
			...
			return (!(mode->channels[i].flag &
				 (HOSTAPD_CHAN_DISABLED |
				  ...
				  HOSTAPD_CHAN_RADAR)) || is_channel_avaliable(wpa_s, scan_res, mode->channels[i].chan));
		}
	}
				   ...
				   u8 channel, u8 bw)
{
	...
	if (!has_channel(wpa_s, wpa_s->global, mode, channel, &flag))
		return -1;
	if (bw == BW40MINUS &&
	    (!(flag & HOSTAPD_CHAN_HT40MINUS) ||
	     !has_channel(wpa_s, wpa_s->global, mode, channel - 4, NULL)))
		return 0;
	if (bw == BW40PLUS &&
