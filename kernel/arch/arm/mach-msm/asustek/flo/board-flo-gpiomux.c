...
void __init apq8064_init_gpiomux(void)
{
	...
	if (machine_is_apq8064_flo() || machine_is_apq8064_deb()) {
		if (revision == HW_REV_B) {
			pr_info("Enable PULL_UP for PCB_ID2 and PCB_ID3\n");
			/*
			 * No external resistors are pulled so that internal
			 * weak pull-up resistor is used instead for PCB_ID[3:2]
			 */
			asustek_pcbid_pins_configs[2].
				settings[GPIOMUX_ACTIVE]->
				pull = GPIOMUX_PULL_UP;
			asustek_pcbid_pins_configs[2].
				settings[GPIOMUX_SUSPENDED]->
				pull = GPIOMUX_PULL_UP;
			asustek_pcbid_pins_configs[3].
				settings[GPIOMUX_ACTIVE]->
				pull = GPIOMUX_PULL_UP;
			asustek_pcbid_pins_configs[3].
				settings[GPIOMUX_SUSPENDED]->
