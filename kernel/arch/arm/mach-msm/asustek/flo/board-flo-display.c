...
#define gpio_LCD_BL_EN_SR1 PM8921_GPIO_PM_TO_SYS(30)	//panel enable and backlight enable
#define gpio_LCD_BL_EN_SR2 PM8921_GPIO_PM_TO_SYS(36)	//panel enable and backlight enable
...
static uint32_t display_gpio_table_SR1[] = {
	/* LCM_XRES */
	GPIO_CFG(gpio_LCM_XRES_SR1, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	/* LCM_TE */
	//GPIO_CFG(gpio_LCM_TE, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};

static uint32_t display_gpio_table_SR2[] = {
	/* LCM_XRES */
	GPIO_CFG(gpio_LCM_XRES_SR2, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	/* LCM_TE */
	//GPIO_CFG(gpio_LCM_TE, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		if (type==0) {	//only for JDI panel
			...
			//LCM_TE
			/*rc = gpio_request(gpio_LCM_TE, "LCM_TE");
			if (rc) {
				pr_err("request gpio 0 failed, rc=%d\n", rc);
				return -ENODEV;
			}*/
			//config LCM_XRES and LCM_TE for JDI panel
			if(hw_revision==0) {
				for (n = 0; n < ARRAY_SIZE(display_gpio_table_SR1); n++) {
					rc = gpio_tlmm_config(display_gpio_table_SR1[n], GPIO_CFG_ENABLE);
					if (rc) {
						pr_err("%s: gpio_tlmm_config(%#x)=%d\n",
							__func__, display_gpio_table_SR1[n], rc);
						break;
					}
				}
			} else {
				for (n = 0; n < ARRAY_SIZE(display_gpio_table_SR2); n++) {
					rc = gpio_tlmm_config(display_gpio_table_SR2[n], GPIO_CFG_ENABLE);
					if (rc) {
						pr_err("%s: gpio_tlmm_config(%#x)=%d\n",
							__func__, display_gpio_table_SR2[n], rc);
						break;
					}
				}
...
static int hdmi_enable_5v(int on)
{
//comment this for flo and deb use slimport as HDMI output
#if 0
...
void __init apq8064_init_fb(void)
{
	...
	if (type==3) {
		...
		printk("%s: register mipi_JDI_panel_device\n", __func__);
