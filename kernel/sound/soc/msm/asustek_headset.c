...
static void 		detection_work(struct work_struct *work);
static void		detection_debug_remove_work(struct work_struct *work);
...
#define DB_TX_GPIO      (82)
#define DB_RX_GPIO      (83)
#define JACK_GPIO	(45)
...
static struct gpiomux_setting pulllow_config = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_8MA,
	.pull = GPIOMUX_PULL_NONE,
};

static struct gpiomux_setting txpullnone_config = {
	.func = GPIOMUX_FUNC_2,
	.drv = GPIOMUX_DRV_8MA,
	.pull = GPIOMUX_PULL_NONE,
};

static struct gpiomux_setting rxpullnone_config = {
	.func = GPIOMUX_FUNC_1,
	.drv = GPIOMUX_DRV_8MA,
	.pull = GPIOMUX_PULL_NONE,
};
...
static struct workqueue_struct *g_detection_debug_remove_queue;
static DECLARE_WORK(g_detection_work, detection_work);
static DECLARE_DELAYED_WORK(g_detection_debug_remove_work, detection_debug_remove_work);
...
static void pull_low_uart_gpio(int status)
{
	if (status == ON) {
		msm_gpiomux_write(DB_TX_GPIO, GPIOMUX_ACTIVE,
				    &pulllow_config, NULL);
		msm_gpiomux_write(DB_TX_GPIO, GPIOMUX_SUSPENDED,
				    &pulllow_config, NULL);
		msm_gpiomux_write(DB_RX_GPIO, GPIOMUX_ACTIVE,
				    &pulllow_config, NULL);
		msm_gpiomux_write(DB_RX_GPIO, GPIOMUX_SUSPENDED,
				    &pulllow_config, NULL);
		return;
	} else {
		msm_gpiomux_write(DB_TX_GPIO, GPIOMUX_ACTIVE,
				    &txpullnone_config, NULL);
		msm_gpiomux_write(DB_TX_GPIO, GPIOMUX_SUSPENDED,
				    &txpullnone_config, NULL);
		msm_gpiomux_write(DB_RX_GPIO, GPIOMUX_ACTIVE,
				    &rxpullnone_config, NULL);
		msm_gpiomux_write(DB_RX_GPIO, GPIOMUX_SUSPENDED,
				    &rxpullnone_config, NULL);
		return;
	}
...
static void detection_debug_remove_work(struct work_struct *work)
{
	if(gpio_get_value(DB_DET_GPIO) == 1) {
		printk("debug board is removed\n");
		pull_low_uart_gpio(ON);
		while(disable_count){
			enable_irq(hs_data->hp_det_irq);
			enable_irq_wake(hs_data->hp_det_irq);
			disable_count--;
		}
		cancel_delayed_work(&g_detection_debug_remove_work);
	} else {
		queue_delayed_work(g_detection_debug_remove_queue,
			&g_detection_debug_remove_work, 3*HZ);
	}
}
...
static void detection_work(struct work_struct *work)
{
	...
	if (machine_is_apq8064_duma()) {
		if (gpio_get_value(DB_DET_GPIO) == 0) {
			printk("%s debug board\n", __func__);
			pull_low_uart_gpio(OFF);
			disable_count = disable_count +2;
			disable_irq(hs_data->hp_det_irq);
			disable_irq_wake(hs_data->hp_det_irq);
			queue_delayed_work(g_detection_debug_remove_queue,
				&g_detection_debug_remove_work, HZ);
			goto closed_micbias;
		}
	}
...
static int __init headset_init(void)
{
	...
	g_detection_debug_remove_queue =
		create_freezable_workqueue("detection_debug_remove");
