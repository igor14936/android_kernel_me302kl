...
#include "wcd9310.h"
#include "../../../arch/arm/mach-msm/asustek/flo/board-flo.h"
...
static int codec_ctl_status = 0;
static ssize_t codec_ctl_status_show(struct device *dev,struct device_attribute *attr,
                                        char *buf);

DEVICE_ATTR(wcd9310_ctl_status, S_IRUGO, codec_ctl_status_show, NULL);
static unsigned int hw_revision = HW_REV_INVALID;
extern void wcnss_s2_power(int);
...
static void check_codec_ctl_status(void)
{
	...
	if (wcd9310_codec != NULL) {
		/* use update bits to test read/write function */
		ret = snd_soc_update_bits(wcd9310_codec, TABLA_A_MICB_3_CTL,
			0x01, 0x01);
		if (ret < 0) {
			codec_ctl_status = 0;
			printk("%s: wcd9310 codec update fail\n", __func__);
		} else {
			/* set register value back */
			snd_soc_update_bits(wcd9310_codec, TABLA_A_MICB_3_CTL,
                        0x01, 0x00);
			if (ret < 0) {
				codec_ctl_status = 0;
				printk("%s: set value back fail\n", __func__);
			} else
				codec_ctl_status = 1;
		}
	} else
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		/* set micbias1 cap mode = External bypass cap */
		snd_soc_update_bits(codec, TABLA_A_MICB_1_CTL, 0x10, 0x0);
		break;
	}
	return 0;
}
...
//	snd_soc_dapm_add_routes(dapm, audio_map, ARRAY_SIZE(audio_map));
	...
	ret = device_create_file(codec->dev, &dev_attr_wcd9310_ctl_status);
	if (ret != 0) {
	dev_err(codec->dev,
		"Failed to create hp_uart_switch sysfs files: %d\n", ret);
	}
